...
const char * const BITCOIN_CONF_FILENAME = "denizlite.conf";
const char * const BITCOIN_PID_FILENAME = "denizlite.pid";
...
#else
    const char* pszModule = "denizlite";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Denizlite";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Denizlite";
#else
    // Unix
    return pathRet / ".denizlite";
#endif
#endif
}
