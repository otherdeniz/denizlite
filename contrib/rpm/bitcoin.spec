...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 4126
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 4127
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 14126
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 14127
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 4126
	%{_sbindir}/semanage port -d -p tcp 4127
	%{_sbindir}/semanage port -d -p tcp 14126
	%{_sbindir}/semanage port -d -p tcp 14127
	for selinuxvariant in %{selinux_variants}; do
